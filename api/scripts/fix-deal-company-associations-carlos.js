// Script de un solo uso: asocia los Deals importados (Carlos) con su Empresa
...
//   cd bepharma-crm
//   node api/scripts/fix-deal-company-associations-carlos.js --dry-run
//   node api/scripts/fix-deal-company-associations-carlos.js --confirm
//
// Lee el token desde bepharma-crm/.env (HUBSPOT_ACCESS_TOKEN) y el mapeo
// deal->dominio desde api/scripts/data/deals_empresa_map_carlos.json (ya
...
// empresas de Carlos (bp_zona='carlos') quedaron con la propiedad ESTANDAR
...
// Y se filtra SOLO a empresas con bp_zona='carlos', para no cruzar nunca con
// empresas de otro operador. Si dos empresas de Carlos comparten el mismo
...
async function fetchCompanyDomainToId() {
  // domain -> companyId, SOLO para empresas de bp_zona='carlos' (evita
  ...
  while (true) {
    ...
    process.stdout.write(`\r  Empresas leidas (total portal): ${leidas}, con zona=carlos y dominio: ${map.size}…`)
  ...
  for (const [domain, count] of seenCount) {
    if (count > 1) map.set(domain, null) // ambiguo: dos empresas de Carlos con el mismo dominio
  }
  return map
}
...
async function main() {
  ...
  console.log(`Total empresas con dominio (zona carlos): ${companyByDomain.size}\n`)
  ...
  for (const row of rows) {
    ...
    if (companyId === null) { dominioAmbiguo++; continue } // dos empresas de Carlos comparten este dominio
  ...
  console.log(`Dominio sin empresa correspondiente (zona carlos) en HubSpot: ${dominioSinEmpresa}`)
  console.log(`Dominio ambiguo (2+ empresas de Carlos con el mismo dominio): ${dominioAmbiguo}`)
