// Script de un solo uso: asocia los Deals importados (Yesenia) con su Empresa
...
//   cd bepharma-crm
//   node api/scripts/fix-deal-company-associations-yesenia.js --dry-run
//   node api/scripts/fix-deal-company-associations-yesenia.js --confirm
//
// Lee el token desde bepharma-crm/.env (HUBSPOT_ACCESS_TOKEN) y el mapeo
// deal->dominio desde api/scripts/data/deals_empresa_map_yesenia.json (ya
...
// empresas de Yesenia (bp_zona='yesenia') quedaron con la propiedad ESTANDAR
...
// Y se filtra SOLO a empresas con bp_zona='yesenia', para no cruzar nunca con
// empresas de otro operador. Si dos empresas de Yesenia comparten el mismo
...
async function fetchCompanyDomainToId() {
  // domain -> companyId, SOLO para empresas de bp_zona='yesenia' (evita
  ...
  while (true) {
    ...
    process.stdout.write(`\r  Empresas leidas (total portal): ${leidas}, con zona=yesenia y dominio: ${map.size}…`)
  ...
  for (const [domain, count] of seenCount) {
    if (count > 1) map.set(domain, null) // ambiguo: dos empresas de Yesenia con el mismo dominio
  }
  return map
}
...
async function main() {
  ...
  console.log(`Total empresas con dominio (zona yesenia): ${companyByDomain.size}\n`)
  ...
  for (const row of rows) {
    ...
    if (companyId === null) { dominioAmbiguo++; continue } // dos empresas de Yesenia comparten este dominio
  ...
  console.log(`Dominio sin empresa correspondiente (zona yesenia) en HubSpot: ${dominioSinEmpresa}`)
  console.log(`Dominio ambiguo (2+ empresas de Yesenia con el mismo dominio): ${dominioAmbiguo}`)
