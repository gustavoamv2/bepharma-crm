// Script de un solo uso: importa la hoja MOVIMIENTOS_DEAL (Angel) a HubSpot
...
// bloqueo es una validacion, no crea nada -- se confirmo con la API que no
// hay notas ni deals corruptos de ese intento.
//
...
const DATA_FILE = process.argv.find(a => a.startsWith('--data='))
  ...
  : path.join(__dirname, 'data', 'movimientos_angel.json')

const DRY_RUN = !process.argv.includes('--confirm')
const BATCH_SIZE = 100
const REQUEST_DELAY_MS = 200

// Owner de Angel (ver mapa de owners en memoria del proyecto BePharma).
// Si este script se reusa para otro operador, cambiar este ID.
const OPERATOR_OWNER_ID = '93771980'
...
async function fetchDealIndex(companyDomains) {
  ...
  while (true) {
    ...
    for (const r of results) {
      ...
      const compAssoc = r.associations && r.associations.companies && r.associations.companies.results
        ? r.associations.companies.results[0] : null
      if (compAssoc) {
  ...
  console.log('')
  return { byNameAndDomain: byNameAndDomain, nameCount: nameCount, byNameOnly: byNameOnly }
}
...
async function createNoteBatch(items) {
  const inputs = items.map(function (item) {
    return {
      properties: {
...
async function main() {
  if (!fs.existsSync(DATA_FILE)) {
    console.error('No se encontro el archivo de datos: ' + DATA_FILE)
    process.exit(1)
  }
  ...
  let totalDeals = 0
  index.nameCount.forEach(function (v) { totalDeals += v })
  console.log('Total deals en HubSpot: ' + totalDeals + ' (' + index.nameCount.size + ' nombres unicos)\n')
  ...
  for (const mov of movimientos) {
    const r = resolveDealId(mov, index)
    if (r.ambiguous) {
    ...
    matched.push({ dealId: r.id, hs_note_body: mov.hs_note_body, timestamp: ts })
  }
  ...
  if (sinDeal.length > 0) {
    ...
    Array.from(new Set(sinDeal)).slice(0, 10).forEach(function (n) { console.log('  - ' + n) })
  }
  if (ambiguos.length > 0) {
    console.log('\nEmpresas duplicadas en HubSpot (mismo nombre de deal, revisar y fusionar antes de reintentar):')
    Array.from(new Set(ambiguos)).forEach(function (n) { console.log('  - ' + n) })
  }

  if (DRY_RUN) {
    console.log('\n--dry-run activo: no se creo ninguna nota. Corre con --confirm para crearlas de verdad.')
    return
  }
  ...
  if (!ok) {
    console.log('Cancelado -- no se creo ninguna nota.')
    return
  }
  ...
  for (let i = 0; i < matched.length; i += BATCH_SIZE) {
    ...
    try {
      ...
      console.error('\nError en lote ' + (i / BATCH_SIZE + 1) + ':', (e.response && e.response.data) || e.message)
