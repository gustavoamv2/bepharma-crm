...
# -*- coding: utf-8 -*-
"""
Transforma los Excel de campo de Carlos (Directorio/No participan/Publicos...)
al formato de importacion BePharma (Empresas/Contactos/Deals/Movimientos).

Version mejorada respecto al script de Angel (03-jul-2026):
- Fix columnas posicionales: los archivos de Carlos NO tienen columnas extra
  de "comentario telefono/email empresa" despues de Tel empresa/Email empresa
  (van seguidas de E-mail persona / LKD normalmente) -- la regla posicional
  ahora solo aplica si esa columna esta en blanco o duplica un header ya visto
  antes en la fila (asi no se pisa E-mail persona / LKD con datos de otra cosa).
- Fix dedupe: se unifico el registro por dominio y por nombre para detectar
  correctamente cuando la MISMA empresa aparece una vez con dominio y otra
  vez sin dominio (bug real encontrado en el batch de Angel: 8 empresas
  quedaron duplicadas porque estaban en Directorio Y en No participan del
  mismo pais).
"""
import glob
import os
...
SRC_DIR = "Carlos"
OUT_XLSX = "BePharma_Excel_Importacion_Carlos.xlsx"
OPERATOR_EMAIL = "information@bepharma.org"
OPERATOR_ZONE = "carlos"
OPERATOR_PREFIX = "BP-CAR"
EVENT_CODE = "BEPH-2026-09"
PIPELINE_ID = "908086638"

# Orden de paises segun sorted(glob(...)) -- evita tener que escribir a mano
# nombres de archivo con acentos/caracteres raros (ej. "Sudаfrica" tiene una
# 'a' cirilica en el nombre real del archivo).
COUNTRY_ORDER = [
    "Alemania", "Austria", "Brasil", "Canada", "Eslovaquia", "Francia",
    "Islandia", "Israel", "Marruecos", "Moldavia", "Portugal", "Qatar",
    "Serbia", "Sudafrica", "Suiza", "Ucrania",
]

COUNTRY_REGION = {
    "alemania": "europa_occ", "austria": "europa_occ", "francia": "europa_occ",
    "islandia": "europa_occ", "portugal": "europa_occ", "suiza": "europa_occ",
    "brasil": "latam_sur",
    "canada": "norteamerica",
    "eslovaquia": "europa_oriental", "moldavia": "europa_oriental",
    "serbia": "europa_oriental", "ucrania": "europa_oriental",
    "israel": "medio_oriente", "qatar": "medio_oriente",
    "marruecos": "africa", "sudafrica": "africa",
}
...
def build_file_country_map(files):
    mapping = {}
    if len(files) != len(COUNTRY_ORDER):
        print("ADVERTENCIA: %d archivos encontrados pero %d paises en COUNTRY_ORDER -- revisar mapeo a mano" % (len(files), len(COUNTRY_ORDER)))
    for f, country in zip(files, COUNTRY_ORDER):
        mapping[os.path.basename(f)] = country
    return mapping
...
def build_column_roles(header_row_vals):
    ...
    for i, h in enumerate(normed):
        ...
        # Solo se pisa esta columna con el rol de "comentario empresa" si esta
        # en blanco o si su texto ya aparecio antes en la fila (duplicado) --
        # si es un header nuevo y con nombre real (ej. "E-mail persona", "LKD"),
        # se respeta su mapeo normal.
...
def process_workbook(path, file_country_map, log):
    ...
    country_default = file_country_map.get(fname, "")
...
def main():
    ...
    domain_registry = {}   # dominio -> [nombres aceptados]
    name_registry = {}     # nombre normalizado -> [{"domain":..,"name":..}]
    empresas_rows = []
    contactos_rows = []
    deals_rows = []
    movimientos_rows = []
