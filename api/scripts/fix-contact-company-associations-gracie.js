// Script de un solo uso: asocia los Contactos importados (Gracie) con su
...
// 971 empresas de Gracie (bp_zona = 'carlos') tienen 0 contactos asociados.
//
...
//   cd bepharma-crm
//   node api/scripts/fix-contact-company-associations-gracie.js --dry-run
//   node api/scripts/fix-contact-company-associations-gracie.js --confirm
//
// Lee el token desde bepharma-crm/.env (HUBSPOT_ACCESS_TOKEN) y el mapeo
// contacto->dominio desde api/scripts/data/contactos_empresa_map_gracie.json
...
// empresas de Gracie quedaron con la propiedad ESTANDAR "domain" vacia
...
// Y se filtra SOLO a empresas con bp_zona='gracie'. Si dos empresas de
...
const DATA_FILE = process.argv.find(a => a.startsWith('--data='))
  ...
  : path.join(__dirname, 'data', 'contactos_empresa_map_gracie.json')
